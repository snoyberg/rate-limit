...
drainQueue queue = do
  rl <- newRateLimit 4 1000000
  replicateConcurrently_ workerCount (drainQueueWorker queue rl)

drainQueueWorker :: TBMQueue Request -> RateLimit -> IO ()
drainQueueWorker urlQueue rl = do
